...
class Teams(BaseModel, SoftDeleteModel):
    ...
    team_leader = models.ForeignKey('auth.User', related_name='%(class)s_team_leader', on_delete=models.RESTRICT, null=True)
    def __str__(self):
        return self.team_name + " - " + self.team_leader.username
    def get_queryset(self):
        superQuery = super(Teams, self).get_queryset().filter(deleted_at__isnull=False)
        print (superQuery)
        return superQuery

# model to accept media files like images, videos, pdfs etc.
class MediaStore(BaseModel, SoftDeleteModel):
    media_file = models.FileField(upload_to='comment_attachments/')
...
class Bug(BaseModel, SoftDeleteModel):
    ...
    media = models.ManyToManyField(MediaStore, related_name='%(class)s_media', blank=True)
...
class BugResolution(BaseModel, SoftDeleteModel): 
    ...
    bug = models.OneToOneField(Bug, related_name='%(class)s_bug', on_delete=models.RESTRICT, null=True)
...
class BugDuplicate(BaseModel, SoftDeleteModel):
    ...
    def __str__(self):
        return str(self.duplicate_id) + " - " + self.parent.title + " - " + self.child.title
